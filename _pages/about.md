---
layout: about
title: About
permalink: /

profile:
  align: right
  image: prof_pic.jpg
  image_circular: false # crops the image to make it circular
  # more_info: >

news: true # includes a list of news items
# latest_posts: true # includes a list of the newest posts
# selected_papers: true # includes a list of papers marked as "selected={true}"
social: true # includes social icons at the bottom of the page
---
I am an Assistant Professor at [Vachani School of Advanced Computing](https://www.ashoka.edu.in/vachani-school-of-advanced-computing/), [Ashoka University](https://www.ashoka.edu.in/). My research spans different aspects of natural language processing, graph-structured modeling, and causal machine learning. My goal is to develop methods that extract structure and meaning from complex, unstructured text to address problems of real societal consequence. I focus on two broad themes:

- **Extracting structure from text to support decisions.** I develop methods to model temporally and causally grounded knowledge from unstructured, longitudinal text to improve decision support and social systems;  for example, extracting patient timelines to predict early disease risk, or analyzing language to understand how it reflects and shapes human behavior, such as group conformity and polarization.
- **Making language model behavior trustworthy.** I study how design and alignment choices in LLMs give rise to bias and vulnerabilities in high-stakes decision-making, and build NLP systems that are not only accurate but also fair, efficient, reliable and secure.

I am motivated by compelling problems, and some of the domains that I have studied include clinical NLP, computational social science, climate-risk and critical infrastructure decision support, and behavioral finance.

Before joining Ashoka, I was a Postdoctoral Fellow at [Kellogg School of Management](https://www.kellogg.northwestern.edu/) at Northwestern University and the [Northwestern Institute on Complex Systems (NICO)](https://www.nico.northwestern.edu/), and at the [Mathematics and Computer Science Division](https://www.anl.gov/mcs) at [Argonne National Laboratory](https://www.anl.gov/). I earned my Ph.D. and Master's in Computer Science from the [University of Illinois Chicago](https://cs.uic.edu/). 

Before doctoral studies, I served as an Assistant Professor of Computer Science at Keshav Mahavidyalaya, University of Delhi, and worked as an Associate Software Engineer at Objective Systems Integrators. I hold prior Master's degree in Computer Applications from Guru Gobind Singh Indraprastha University, and Bachelor's degree in Physics from University of Delhi.
 